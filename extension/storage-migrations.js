(function (root, factory) {
  const identityMigration = root && root.FollowTrackerStorageIdentityMigration
    ? root.FollowTrackerStorageIdentityMigration
    : (typeof module === "object" && module.exports ? require("./storage-identity-migration.js") : null);
  const api = factory(identityMigration);
  if (typeof module === "object" && module.exports) module.exports = api;
  if (root) root.FollowTrackerStorageMigrations = api;
})(typeof globalThis !== "undefined" ? globalThis : this, function (IdentityMigration) {
  "use strict";
  if (!IdentityMigration) throw new Error("Follow Tracker Storage Migrations no pudo cargar Identity Migration.");
  ...
  function planStorageMigration(itemsValue, options) {
    ...
    if (fromVersion < 3) {
      ...
      Object.keys(items).filter((key) => key.startsWith("ft_identity_")).forEach((identityKey) => {
        ...
        setValue(identityKey, IdentityMigration.migrateIdentityRegistry(items[identityKey], profile));
      });
    }
  ...
  return { DEFAULT_SETTINGS, MIGRATION_BACKUP_KEY, STORAGE_META_KEY, STORAGE_SCHEMA_VERSION, checksum, currentVersion,
    migrateIdentityRegistry: IdentityMigration.migrateIdentityRegistry, migrateStorage, normalizeSettings, planStorageMigration,
    restoreMigration, validateBackup, validateStorageSnapshot };
});