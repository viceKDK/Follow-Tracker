(function (root, factory) {
  ...
  if (root) root.FollowTrackerInstagramApi = api;
  ...
  function profileFromLocation(locationValue) {
    ...
    return /^[a-zA-Z0-9._]+$/.test(first) && !blocked.has(first.toLowerCase())
      ? first.toLowerCase()
      : "";
  }
  ...
  function headers() {
    return {
      ...
      "x-asbd-id": "129477",
      "x-ig-www-claim": cookie("ig_www_claim") || "0",
    ...
    for (let attempt = 1; attempt <= settings.maxAttempts; attempt += 1) {
      ...
      try {
        const response = await fetch(url, {
          method: "GET",
          credentials: "include",
          headers: { ...headers(), referer: settings.referer || `${location.origin}/` },
          signal: controller.signal,
        });
        if (response.ok) return await response.json();
        if (response.status === 401 || response.status === 403) {
          throw new Error("Instagram pidió volver a iniciar sesión o no permite ver esas listas.");
        }
        if ([429, 502, 503, 504].includes(response.status)) {
          ...
          const wait = Number.isFinite(retryHeader) && retryHeader > 0
            ? retryHeader
            : Math.min(settings.maxBackoffMs, 1200 * Math.pow(2, attempt - 1));
        ...
        if (error && error.name === "AbortError") {
          lastError = new Error("Instagram demoró demasiado en responder.");
        } else {
          lastError = error;
        }
  ...
  async function profileInfo(profile, options) {
    const json = await requestJson(
      `/api/v1/users/web_profile_info/?username=${encodeURIComponent(profile)}`,
      {
        ...(options || {}),
        referer: `${location.origin}/${profile}/`,
      }
    );
    ...
    if (user.is_private && !user.followed_by_viewer) {
      throw new Error("La cuenta es privada y la sesión actual no puede ver sus listas.");
    }
    return {
      id: String(user.id),
      username: Trust.normalizeUsername(user.username || profile),
      isPrivate: Boolean(user.is_private),
      followersCount: Number.isFinite(Number(user.edge_followed_by && user.edge_followed_by.count))
        ? Number(user.edge_followed_by.count)
        : null,
      followingCount: Number.isFinite(Number(user.edge_follow && user.edge_follow.count))
  ...
  async function collectPhase(userId, phase, expected, profile, options) {
    ...
    while (pages < settings.maxPages) {
      ...
      const json = await requestJson(`/api/v1/friendships/${userId}/${endpoint}/?${query}`, {
        ...settings,
        referer: `${location.origin}/${profile}/${endpoint}/`,
        onRetry(info) {
          retries += 1;
          settings.onRetry && settings.onRetry({ ...info, phase });
        },
      });
      ...
      if (!pageUsers.length || added === 0) emptyPages += 1;
      else emptyPages = 0;
    ...
    return { users: Trust.uniqueUsers(users, "api"), pages, retries };
  }
  ...
  async function collectProfile(profileValue, options) {
    ...
    if (followerCoverage != null && followerCoverage < 0.95) {
      warnings.push(`Instagram entregó ${followers.users.length} de ${info.followersCount} seguidores.`);
    }
    if (followingCoverage != null && followingCoverage < 0.95) {
      warnings.push(`Instagram entregó ${following.users.length} de ${info.followingCount} seguidos.`);
    }
    return {
      source: "api",
      profile,
      ...
      expectedFollowers: info.followersCount,
      expectedFollowing: info.followingCount,
      durationMs: Date.now() - startedAt,
      retries: followers.retries + following.retries,
      warnings,
    };
  }
