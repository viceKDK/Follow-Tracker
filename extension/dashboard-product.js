...
(function () {
  ...
  const activityAssets = {
    followed_you: "icons/metric-followers-v2.png",
    unfollowed_you: "icons/metric-unfollowed-v4.png",
    you_followed: "icons/metric-following-v2.png",
    you_unfollowed: "icons/metric-following-only-v4.png",
  };

  function injectCss() {
    if (document.querySelector('link[data-follow-tracker-product="true"]')) return;
    const link = document.createElement("link");
    link.rel = "stylesheet";
    link.href = "dashboard-product.css?v=4.2.0";
    link.dataset.followTrackerProduct = "true";
  ...
  function ensureActivityControls() {
    ...
    list.insertAdjacentHTML(
      ...
      `<div id="activity-controls" class="activity-controls">
        ...
        <details class="advanced-disclosure activity-disclosure">
          <summary>Filtros avanzados</summary>
          <div class="activity-advanced">
            ...
            <button id="clear-activity" class="button button-secondary" type="button">Limpiar filtros</button>
          </div>
        </details>
        <button id="export-activity-view" class="button button-secondary" type="button">Exportar CSV</button>
      </div>
  ...
  function renderActivityEnhanced() {
    ...
    target.innerHTML = `<div class="activity-table-shell"><table class="activity-table"><thead><tr><th>Usuario</th><th>Evento</th><th>Fecha detectada</th><th>Reporte</th><th>Perfil</th></tr></thead><tbody>${page.items
      .map((event) => {
        ...
        const asset = activityAssets[event.type] || "icons/metric-mutual.png";
        return `<tr class="activity-row activity-row-${escapeHtml(meta.tone)}"><th data-label="Usuario">${profileAvatarMarkup(event.username, event.reportId, "activity-profile-avatar")}<span class="activity-person"><strong>@${escapeHtml(event.username)}</strong><small>${escapeHtml(formatDate(event.occurredAt))}</small></span></th><td data-label="Evento"><span class="activity-event-badge ${escapeHtml(meta.tone)}"><img class="activity-event-icon" src="${asset}" alt="" />${escapeHtml(meta.title)}</span></td><td data-label="Fecha detectada">${escapeHtml(formatDate(event.occurredAt))}</td><td data-label="Reporte"><code>${escapeHtml(event.reportId || "sin id")}</code></td><td data-label="Perfil"><a class="profile-link" href="https://www.instagram.com/${encodeURIComponent(event.username)}/" target="_blank" rel="noreferrer">Abrir</a></td></tr>`;
      })
  ...
  function renderPeopleSummary() {
    const panel = document.querySelector("#people .people-panel");
    const projection = requireProjection();
    if (!panel || !projection) return;
    let target = document.querySelector("#people-summary");
    if (!target) {
      panel.insertAdjacentHTML("beforebegin", '<section id="people-summary" class="people-summary" aria-label="Resumen actual de relaciones"></section>');
      target = document.querySelector("#people-summary");
    }
    const summary = projection.summary;
    const lost = (projection.latestChanges?.lostFollowers || []).length;
    const cards = [
      ["Te siguen", summary.followers, "Personas que te siguen ahora", "metric-followers-v2.png", "followers"],
      ["Los seguís", summary.following, "Cuentas que seguís ahora", "metric-following-v2.png", "following"],
      ["Se siguen", summary.mutual, "Seguimiento mutuo", "metric-mutual-v2.png", "mutual"],
      ["Solo te siguen", summary.followerOnly, "Te siguen, pero vos no los seguís", "metric-followers-only-v4.png", "followers-only"],
      ["Solo los seguís", summary.followingOnly, "Vos los seguís, pero ellos no te siguen", "metric-following-only-v4.png", "following-only"],
      ["Te dejaron de seguir", lost, "Desde el reporte anterior", "metric-unfollowed-v4.png", "unfollowed"],
    ];
    target.innerHTML = cards.map(([label, value, note, asset, tone]) => `<article class="people-summary-card people-summary-${tone}"><img src="icons/${asset}" alt="" /><span><small>${escapeHtml(label)}</small><strong>${formatNumber(value)}</strong><em>${escapeHtml(note)}</em></span></article>`).join("");
  }
  ...
  Runtime.on("render:after", renderPeopleSummary, { id: "product.people-summary" });
  Runtime.on("profile:loaded", renderPeopleSummary, { id: "product.people-summary-profile" });
  renderPeopleSummary();
