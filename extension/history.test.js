...
test("conserva avatares en la base y sólo duplica perfiles que cambiaron", () => {
  const first = snapshot({
    users: [
      { id: "1", username: "ana", avatarUrl: "https://cdn.instagram.test/ana.jpg" },
      { id: "2", username: "beto", avatarUrl: "https://cdn.instagram.test/beto.jpg" },
    ],
  });
  const second = snapshot({
    followers: ["ana", "carla"],
    users: [
      { id: "1", username: "ana", avatarUrl: "https://cdn.instagram.test/ana-new.jpg" },
      { id: "3", username: "carla", avatarUrl: "https://cdn.instagram.test/carla.jpg" },
    ],
    updatedAt: "2026-08-21T15:30:00.000Z",
    runId: "run-2",
  });
  ...
  assert.equal(timeline.baseline.users.length, 2);
  assert.deepEqual(timeline.reports[1].users.map((user) => user.username), ["beto", "carla"]);
  const reconstructed = History.snapshotForReport(timeline, "run-2");
  assert.equal(reconstructed.users.find((user) => user.username === "carla").avatarUrl, "https://cdn.instagram.test/carla.jpg");
});
