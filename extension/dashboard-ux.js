...
(function () {
  ...
  function injectCss() {
    if (document.querySelector('link[data-follow-tracker-ux="true"]')) return;
    const link = document.createElement("link");
    link.rel = "stylesheet";
    link.href = "dashboard-ux.css?v=4.2.0";
    link.dataset.followTrackerUx = "true";
  ...
  function ensureControls() {
    ...
    if (tools && !document.querySelector("#relationship-advanced")) {
      tools.insertAdjacentHTML(
        "beforeend",
        `<details id="relationship-advanced-disclosure" class="advanced-disclosure">
          <summary>Más filtros y opciones</summary>
          <div id="relationship-advanced" class="advanced-filter-bar">
          <label><span>Estado actual</span><select id="relationship-state-filter"><option value="all">Todos</option><option value="mutual">Se siguen los dos</option><option value="follows_you">Solo te sigue</option><option value="you_follow">Solo lo seguís</option><option value="none">No se siguen</option></select></label>
          ...
          <label><span>Relaciones por página</span><select id="relationship-page-size"><option value="100">100</option><option value="250" selected>250</option><option value="500">500</option></select></label>
        ...
        <div class="relationship-list-actions">
          <div><strong id="relationship-visible-count">0 relaciones</strong><small>Tocá una persona para ver el detalle.</small></div>
    ...
    if (peopleTools && !document.querySelector("#people-table-options")) {
      peopleTools.insertAdjacentHTML(
        "afterend",
        `<details id="people-options-disclosure" class="advanced-disclosure">
          <summary>Opciones de la lista</summary>
  ...
  function renderRelationshipTable() {
    ...
    if (count) {
      count.textContent = page.total
        ? `${formatNumber(page.start)}–${formatNumber(page.end)} de ${formatNumber(page.total)} relaciones`
        : "0 relaciones";
    }
    ...
    const columns = [
      { state: "mutual", title: "Se siguen los dos", description: "Ambos se siguen", icon: "icons/metric-mutual-v2.png" },
      { state: "follows_you", title: "Solo te sigue", description: "Te sigue. Vos no lo seguís.", icon: "icons/metric-followers-only-v4.png" },
      { state: "you_follow", title: "Solo lo seguís", description: "Lo seguís. No te sigue.", icon: "icons/metric-following-only-v4.png" },
      { state: "none", title: "No se siguen", description: "Sin seguimiento actual", icon: "icons/metric-unfollowed-v4.png" },
    ];
    target.innerHTML = `<div class="relationship-board">${columns.map((column) => {
      const items = page.items.filter((item) => item.toState === column.state);
      return `<section class="relationship-column relationship-column-${column.state}" aria-label="${escapeHtml(column.title)}">
        <header class="relationship-column-head"><img src="${column.icon}" alt="" /><span><strong>${escapeHtml(column.title)}</strong><small>${escapeHtml(column.description)}</small></span><b>${formatNumber(items.length)}</b></header>
        <div class="relationship-column-list">${items.length ? items.map((item) => `<article class="relationship-person-card table-tone-${resultTone(item.tone)} clickable-table-row" data-user="${escapeHtml(item.normalized)}" data-source="comparison" tabindex="0">
          ${profileAvatarMarkup(item.normalized, state.compareTo)}
          <span class="relationship-person-main"><strong>@${escapeHtml(item.username)}</strong><small>${escapeHtml(item.headline)}</small></span>
          <a class="profile-link" href="https://www.instagram.com/${encodeURIComponent(item.normalized)}/" target="_blank" rel="noreferrer" aria-label="Abrir perfil de @${escapeHtml(item.username)}">Ver</a>
        </article>`).join("") : '<p class="relationship-column-empty">Sin personas en esta relación</p>'}</div>
      </section>`;
    }).join("")}</div>${paginator("relationships", page)}`;
  }
  ...
  function renderPeopleTable() {
    ...
    target.innerHTML = `<div class="current-table-shell"><table class="current-table people-current-table"><thead><tr>
      <th>${sortButton("Persona", "username", ux.peopleSort)}</th>
      ...
      .map((person) => {
        ...
        return `<tr class="clickable-table-row" data-user="${escapeHtml(person.username)}" data-source="current" tabindex="0">
          <th class="table-user-cell" data-label="Usuario"><div class="table-user-content">${profileAvatarMarkup(person.username)}<span><strong>@${escapeHtml(person.username)}</strong><small>${person.events.length} cambio${person.events.length === 1 ? "" : "s"}</small></span></div></th>
          <td data-label="Relación actual"><span class="relationship-badge ${escapeHtml(person.relationship)}">${escapeHtml(relationshipLabel(person))}</span><small class="relationship-facts">Te sigue: ${person.followsYou ? "Sí" : "No"} · Lo seguís: ${person.youFollow ? "Sí" : "No"}</small></td>
          <td data-label="Último cambio"><span class="last-event">${last ? escapeHtml(meta.title) : "Sin cambios detectados"}<small>${last ? escapeHtml(formatDate(last.occurredAt)) : `${formatNumber(person.events.length)} cambios guardados`}</small></span></td>
