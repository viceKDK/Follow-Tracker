(function (root, factory) {
  ...
  function parseCount(text) {
    ...
    if (abbreviated) { const number = Number(abbreviated[1].replace(",", ".")); return Number.isFinite(number) ? Math.round(number * (abbreviated[2].toLowerCase() === "m" ? 1000000 : 1000)) : null; }
  ...
  function expectedInfoFromTrigger(trigger) {
    if (!trigger) return { value: null, exact: false };
    const titled = trigger.querySelector("[title]") || (trigger.hasAttribute("title") ? trigger : null);
    const titleText = titled ? String(titled.getAttribute("title") || "") : "";
    const text = titleText || String(trigger.textContent || "");
    return { value: parseCount(text), exact: !/[0-9](?:[.,][0-9]+)?\s*[km]\b/i.test(text) };
  }

  function expectedFromTrigger(trigger) { return expectedInfoFromTrigger(trigger).value; }
  ...
  async function waitFor(predicate, timeoutMs, signal) {
    ...
    while (Date.now() - started < timeoutMs) {
      ...
      const value = predicate(); if (value) return value; await sleep(150, signal);
    }
    return null;
  }
  ...
  function userFromHref(href) {
    ...
    if (/^https?:/i.test(path)) { try { path = new URL(path).pathname; } catch (_error) { return null; } }
  ...
  function collectVisible(scope, target) {
    ...
    scope.querySelectorAll("a[href]").forEach((anchor) => {
      ...
      target.set(username, Trust.normalizeUser({ username,
        ...
        profilePicUrl: image && image.currentSrc || image && image.src || "" }, "ui"));
      added += 1;
    });
    if (!added) {
      scope.querySelectorAll("li,div[role='listitem'],div[role='button']").forEach((row) => {
        ...
        target.set(username, Trust.normalizeUser({ username, fullName: lines[1] || "",
          profilePicUrl: image && image.currentSrc || image && image.src || "" }, "ui")); added += 1;
      });
    }
    return added;
  }
  ...
  function scrollScore(element) {
    ...
    const scrollable = element.scrollHeight > element.clientHeight + 20 || ["auto", "scroll"].includes(style.overflowY);
    return (scrollable ? 100 : 0) + element.querySelectorAll("a[href]").length * 4 + Math.min(50, element.scrollHeight / 1000);
  }

  function scrollContainer(scope) {
    const candidates = [scope, ...scope.querySelectorAll("div")].filter((element) => element && element instanceof HTMLElement)
  ...
  async function collectPhase(profile, phase, options) {
    ...
    while (iterations < settings.maxIterations && target.size < settings.maxUsers) {
      ...
      collectVisible(opened.scope, target);
      settings.onProgress && settings.onProgress({ phase, count: target.size, expected: opened.expected });
      stagnant = target.size === previousSize ? stagnant + 1 : 0;
      previousSize = target.size;
      iterations += 1;
      if (opened.expectedExact && Number.isFinite(opened.expected) && opened.expected >= 0 && target.size >= opened.expected) { terminationReason = "expected_reached"; break; }
      if (stagnant >= settings.stagnantLimit) { terminationReason = "stagnant"; break; }
    }
    if (!terminationReason) terminationReason = iterations >= settings.maxIterations ? "max_iterations" : target.size >= settings.maxUsers ? "max_users" : "stopped";
  ...
  async function collectProfile(profileValue, options) {
    ...
    return { source: "ui", profile, profileId: "", followers: followers.users, following: following.users,
