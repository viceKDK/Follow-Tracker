...
test("conserva la foto HTTPS y descarta URLs inseguras", () => {
  const user = Trust.normalizeUser({
    id: "77",
    username: "Ana",
    profile_pic_url: "https://cdn.instagram.test/ana.jpg",
  }, "api");
  assert.equal(user.instagramUserId, "77");
  assert.equal(user.avatarUrl, "https://cdn.instagram.test/ana.jpg");
  assert.equal(Trust.normalizeUser({ username: "beto", profile_pic_url: "javascript:alert(1)" }).avatarUrl, "");
});
