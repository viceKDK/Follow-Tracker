...
test("importa archivos oficiales y crea una captura con fuente oficial", async ({ page }) => {
  ...
  await page.locator('[data-admin-target="admin-import"]').click();
...
test("elimina un reporte intermedio y reconstruye el timeline", async ({ page }) => {
  const errors = await openDashboard(page, "admin");
  await page.locator('[data-view="admin"]').click();
  await page.locator('[data-admin-target="admin-security"]').click();
  await expect(page.locator('[data-security-report-delete="r2"]')).toBeVisible();
  await page.locator('[data-security-report-delete="r2"]').click();
